...
fun ZoomableCanvas(density: Float, year: Int) {
    ...
    Canvas(
        ...
        drawCanvasContent(this,density,year, scale, offsetX, offsetY, canvasRotation)
    }
}
private fun drawCanvasContent(
    ...
    drawScope.drawIntoCanvas { canvas ->
        ...
        canvas.rotate(-90f)
        for (text in months) {
            canvas.nativeCanvas.drawTextOnPath(text, path, 0f, -230f, paint)
            canvas.rotate(30f)
            when(months.indexOf(text)){
                0 -> canvas.rotate(2f)
